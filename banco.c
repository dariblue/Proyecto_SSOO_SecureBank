...
static int crear_cuenta(Cuenta *nueva) {
    ...
    snprintf(linea, sizeof(linea),
             "[%s] NUEVA CUENTA: id=%d titular=%s",
             ts, nueva->numero_cuenta, nueva->titular);
    escribir_log(linea);
...
static void procesar_log(void) {
    ...
    while (mq_receive(g_mq_log, (char *)&dl, sizeof(dl), NULL) > 0) {
        const char *op_str;
        switch (dl.tipo_op) {
            case OP_DEPOSITO:      op_str = "DEPOSITO";      break;
            case OP_RETIRO:        op_str = "RETIRO";        break;
            case OP_TRANSFERENCIA: op_str = "TRANSFERENCIA"; break;
            case OP_MOVER_DIVISA:  op_str = "MOVER_DIVISA";  break;
            default:               op_str = "DESCONOCIDO";   break;
        }
        char linea[512];
        snprintf(linea, sizeof(linea),
